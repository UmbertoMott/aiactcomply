...
interface DpiaGapCheckProps {
  doc: DPIAResult;
  onNavigateToStep: (step: number) => void;
  onResult?: (result: DpiaGapCheck) => void;
}
...
export function DpiaGapCheck({ doc, onNavigateToStep, onResult }: DpiaGapCheckProps) {
  ...
  async function handleCheck() {
    ...
    const r = await checkDpiaGaps(doc);
    setLoading(false);
  ...
  const statusIcon = (s: DpiaGapItem["status"]) =>
    s === "ok" ? "✓" : s === "incomplete" ? "△" : "✕";
  const statusColor = (s: DpiaGapItem["status"]) =>
    s === "ok" ? T.green : s === "incomplete" ? T.amber : T.red;
  const statusBg = (s: DpiaGapItem["status"]) =>
    s === "ok" ? T.greenBg : s === "incomplete" ? T.amberBg : T.redBg;
  const statusBdr = (s: DpiaGapItem["status"]) =>
      ...
      <div style={{ ...cardSt }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <button
              ...
              style={{
                ...
                border: "none",
                background: loading ? "rgba(0,0,0,0.04)" : T.text,
        ...
        {result && (
          <div style={{ padding: "0 20px 20px" }}>
            {/* Overall badge */}
            <div style={{ padding: "12px 0", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <span style={{
                fontSize: 11, fontWeight: 700, color: overallColor,
                background: `${overallColor}18`, padding: "3px 10px", borderRadius: 9999,
              }}>
                {overallLabel}
              </span>
            ...
            <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
              {result.items.map((item) => (
                <div key={item.articleRef} style={{
                  ...
                  border: `1px solid ${statusBdr(item.status)}`,
                  background: statusBg(item.status),
                  display: "flex", alignItems: "flex-start", gap: 10,
                }}>
                  <span style={{
                    fontSize: 14, fontWeight: 700,
                    color: statusColor(item.status), minWidth: 16, flexShrink: 0,
                  }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: 11, color: T.muted, margin: "2px 0", lineHeight: 1.4 }}>
                      {item.finding}
                    </p>
                    {item.status !== "ok" && item.cta_step >= 0 && (
                      <button
                        onClick={() => onNavigateToStep(item.cta_step)}
                        style={{
                          marginTop: 4, fontSize: 10, fontWeight: 600, color: T.text,
