...
export interface FriaGuidedChatProps {
  doc: FriaGuidedDoc;
  onAnswerUpdate: (subPointId: string, answer: FriaAnswer) => void;
  onNavigateToSubPoint?: (subPointId: string) => void;
  forcedSubPointId?: string | null;
}

export function FriaGuidedChat({
  doc,
  ...
  const currentId = forcedSubPointId
    ...
    ?? nextFriaSubPointId(doc)
    ?? allIds[0];

  const currentIdx = allIds.indexOf(currentId ?? "");
  const sp = FRIA_SUBPOINTS.find(s => s.id === currentId);
